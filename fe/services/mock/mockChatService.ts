...
function reasoningFor(
  ...
  // model nhanh (Flash) chỉ làm 2 bước, model chuyên sâu (Pro/mặc định) làm đủ 4 bước
...
function _questionFor(content: string): { text: string; options: ChoiceOption[] } {
  const text = content.toLowerCase();
  if (text.includes("mụn") || text.includes("trứng cá")) {
    return {
...
function _isDraftingRequest(text: string): boolean {
  const t = text.toLowerCase();
  return DRAFTING_KEYWORDS.some((k) => t.includes(k));
}

/** Sinh mẫu văn bản soạn sẵn (HTML) cho mock */
function _documentFor(content: string): { title: string; html: string } {
