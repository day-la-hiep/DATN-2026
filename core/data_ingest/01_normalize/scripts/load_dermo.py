...
Data lấy từ `../output/kg/dermo/dermo_kg.json` (pipeline: `data-ingest/dermo/` parse
`dermatology.obo` -> `02_normalize_kg.py`, chạy `python data-ingest/run.py dermo normalize`).
Nằm trong build context của `core/Dockerfile`.

Input: `dermo_kg.json` (key `terms`, mỗi term: id/name/def/synonyms/xrefs/is_a/
relationships/alt_ids/is_obsolete — xem `data-ingest/dermo/scripts/`).
