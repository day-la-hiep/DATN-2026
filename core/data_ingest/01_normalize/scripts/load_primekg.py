...
Data lấy từ `../output/kg/primekg/` (pipeline: `data-ingest/primekg/` lọc từ PrimeKG gốc
~850MB -> `02_normalize_kg.py`, chạy `python data-ingest/run.py primekg normalize`). Nằm
trong build context của `core/Dockerfile` nên chạy được cả dev lẫn prod (`docker compose
exec`) — chỉ 2 file cần cho việc NẠP (~2,5MB).

Input: `derma_nodes.csv` (5,2k dòng, cột id/name/type/source) + `derma_edges.csv` (19,7k
dòng, cột relation/display_relation/x_id/x_name/x_type/y_id/y_name/y_type).
...
Chạy:
  cd core && python data-ingest/01_normalize/scripts/load_primekg.py [--reset]
