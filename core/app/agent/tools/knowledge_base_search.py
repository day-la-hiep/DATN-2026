...
pipeline chunking gốc. Chunk được tạo, embed + ingest MỘT LẦN thẳng vào Qdrant qua
`data-ingest/01_normalize/scripts/load_knowledge_base.py` (không đọc file JSON lúc runtime —
...
def get_kb_embeddings() -> LocalEmbeddings:
    """Dùng chung bởi tool này (embed query) và `data-ingest/01_normalize/scripts/load_knowledge_base.py`
