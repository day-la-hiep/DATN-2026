...
class AgentResponseMessage(BaseModel):
    ...
    # (`app/agent/context.py`, `app/agent/worker.py::_drive`) — mỗi dict khớp field của
    # `ReasoningStepDto` (`app/dto/message.py`). Core persist NGUYÊN list này vào
    # `Message.extra.reasoning` (`app/agent/response_consumer.py`) để `GET
