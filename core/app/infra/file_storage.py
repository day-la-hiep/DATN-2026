...
async def get_object_bytes(object_name: str) -> bytes | None:
    ...
    try:
        ...
        # NoSuchKey = key sai/đã xoá (bình thường); các code khác (AccessDenied,
        # InvalidAccessKeyId, NoSuchBucket...) là lỗi cấu hình MinIO — log để phân biệt,
        # vì cả hai đều bị tool báo chung là "không tìm thấy ảnh".
        print(f"[file_storage] get_object failed: code={exc.code} key={object_name!r} bucket={settings.MINIO_BUCKET!r}")
        return None
