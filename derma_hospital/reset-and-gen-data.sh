...
#     -> core/scripts/ingest/ingest_knowledge_base.py
#   - Neo4j:  PrimeKG (quan hệ bệnh-triệu chứng-thuốc) -> core/scripts/ingest/load_primekg.py
#   - Neo4j:  DermO (từ điển thuật ngữ da liễu)         -> core/scripts/ingest/load_dermo.py
#
...
run python scripts/ingest/ingest_knowledge_base.py "${RESET_FLAG[@]}"

echo "── Neo4j: PrimeKG ─────────────────────────────────────────────"
run python scripts/ingest/load_primekg.py "${RESET_FLAG[@]}"

echo "── Neo4j: DermO ────────────────────────────────────────────────"
run python scripts/ingest/load_dermo.py "${RESET_FLAG[@]}"
