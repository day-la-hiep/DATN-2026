...
2 nhóm hàm, 2 collection RIÊNG BIỆT trên cùng client:
  ...
  - `ensure_kb_collection`/`upsert_kb_chunks`/`search_kb_chunks`: guideline da liễu
    ...
    `scripts/ingest/ingest_knowledge_base.py`, đọc bởi
...
async def existing_kb_point_ids(point_ids: list[str]) -> set[str]:
    """Cho ingest script (`scripts/ingest/ingest_knowledge_base.py`) resume được sau khi
...
async def ensure_kb_collection(dim: int) -> None:
    """Tạo collection KB nếu chưa có — gọi bởi `scripts/ingest/ingest_knowledge_base.py`
...
async def delete_kb_collection() -> None:
    ...
    `scripts/ingest/ingest_knowledge_base.py`), vd đổi embedding model/dim. An toàn gọi
